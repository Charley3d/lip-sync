...
class LIPSYNC2D_OT_RemoveLipSync(bpy.types.Operator):
    ...
    def execute(self, context: Context) -> set[
        ...
        if obj is None:
            self.report({'ERROR'}, message="Lip Sync cannot be removed! Please, select an Active Object")

        elif "lipsync2d_props" in obj:

            second_message = ""
            if obj.lipsync2d_props.lip_sync_2d_remove_animation_data:  # type: ignore
                action = bpy.data.actions.get(f"{obj.name}-LipSyncAction")
            ...
            self.report({'INFO'}, message=f"Lip Sync successfully removed! {second_message}")
